...
class ImageApp:
    ...
    def visualize_image(self, img_path, img_bbox, joints):
        # Read the image
    ...
    def load_image(self, file_path):
        ...
        for i, d in enumerate(self.annotations):
            if file_path in d['img_path']:
                print(d)
                print('exits')
                self.image, img_width, img_height = self.visualize_image(file_path, d['img_bbox'], d['joints'] )  # Call visualize_image function
                self.img_width, self.img_height = self.image.size
                self.image_tk = ImageTk.PhotoImage(self.image)
